...
public class RFX1_TransformMotion : MonoBehaviour
{
    ...
    public GameObject[] EffectsOnCollision; // Romeno: left for backward compatibility with the VFX package
    public CollisionVisualEffect[] VFXOnCollision;
    ...
    [HideInInspector] public List<GameObject> CollidedInstances; // Romeno: left for backward compatibility with the VFX package
    [HideInInspector] public List<CollisionVisualEffect> CollidedVFXs;
    ...
    private void Initialize()
    {
        ...
        if (CollidedVFXs == null)
        {
            CollidedVFXs = new List<CollisionVisualEffect>();
        }
        else
        {
            CollidedVFXs.Clear();
    ...
    void UpdateWorldPosition()
    {
        ...
        if (!isOutDistance && currentDistance + RayCastTolerance > Distance)
        {
            ...
            oldPos = t.position;
            SpellProjectile.OnSpellDestroyed();
            return;
        }
    ...
    private IEnumerator DelayedReturnToPool()
    {
        ...
        foreach (var collisionVFX in CollidedVFXs)
        {
    ...
    void OnCollisionBehaviour(RaycastHit hit)
    {
        ...
        // Romeno: added to patch up the package script
        if (VFXOnCollision.Length != 0)
        {
            foreach (var effect in VFXOnCollision)
            {
                var vfx = PoolManager.I.CollisionVFXs.Get(effect.gameObject);
                vfx.transform.position = hit.point + hit.normal * CollisionOffset;
                vfx.transform.rotation = Quaternion.identity;
                CollidedVFXs.Add(vfx);
                if (HUE > -0.9f)
                {
                    var color = vfx.gameObject.AddComponent<RFX1_EffectSettingColor>();
                    var hsv = RFX1_ColorHelper.ColorToHSV(color.Color);
                    hsv.H = HUE;
                    color.Color = RFX1_ColorHelper.HSVToColor(hsv);
                }
                vfx.transform.LookAt(hit.point + hit.normal + hit.normal * CollisionOffset);
                if (!CollisionEffectInWorldSpace) vfx.transform.parent = transform;
            }
